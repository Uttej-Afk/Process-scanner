...
if [ ! -s "$CSV_FILE" ]; then
    echo "Timestamp,Hostname,User,PID,Process_Type,CPU_Percent,Memory_Percent,Duration,Short_Command" > "$CSV_FILE"
fi
...
while read -r user pid pcpu pmem etime comm args; do
    ...
    if [[ -n "$process_type" ]]; then
        ...
        echo "$(escape_csv "$timestamp"),$(escape_csv "$CLUSTER_NAME"),$(escape_csv "$user"),$(escape_csv "$pid"),$(escape_csv "$process_type"),$(escape_csv "$pcpu"),$(escape_csv "$pmem"),$(escape_csv "$etime"),$(escape_csv "$comm")" >> "$CSV_FILE"
